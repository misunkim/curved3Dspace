...
public class mainSpatialTask_ver5 : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
		...
		{	Debug.Log("can't find PID=null");
			...
			text_warning.text="Fatal error! can't proceed to experiment. Contact the experimenter via Prolific message board";	
			PID="null";
        ...
        if (parameters.TryGetValue("distType", out distType)){
            Debug.Log("distType="+distType);
        }else{
            distType="Path";Debug.Log("distType="+distType);
        }
#endif
        ...
        else
        {
            ...
            Debug.Log("load Web/" + subId + "_inputParam_1001");
        }
#endif
        ...
        if (the_JSON_string == "") //if I fail to load the json task param from the server, find it from Resources directory
        {
            ...
            {
                Debug.Log("load Resources/" + subId + "_inputParams_1001");
                the_JSON_string = jsonTextAsset.text;
            }
            else
            {
                text_warning.gameObject.SetActive(true);
                text_warning.text = "Fatal error! can't proceed to experiment. Contact the experimenter via Prolific message";
    ...
    IEnumerator objectDistEstimate_2AFC()
    {
        ...
        if (distType == "Path")
        {
            tmptext = "<b>Distance estimation task:</b>\nIn this task, you should estimate <b>how long it would take to move from one picture cube to other picture cube</b> and compare the distances.";
        }
        else
        {
            if (distType == "Euclid")
    ...
    IEnumerator objectDistEstimate_egocentric()
    {
        ...
        timerSlider.gameObject.SetActive(false);
        img_fullscreen.SetActive(true); //put background image
        tmptext = "<b>Distance estimation task:</b>\nIn this task, you should imagine <b>how long it would take when you move from one picture cube to the other picture cube</b>.";
    ...
    IEnumerator objectDistEstimate_pairwise()
    {
        ...
        if (distType == "Path")
        {
            tmptext = tmptext + "In this task, you should estimate <b>how long it would take to move from one location to other location</b>";
        }
        else
        {
            if (distType == "Euclid")
